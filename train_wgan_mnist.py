...
from gan_model_mnist import Generator, Discriminator
...
OUTPUT_DIM = 784 # Number of pixels in CIFAR10 (3*32*32)
...
def get_args():
    ...
    parser.add_option('--width', dest='width', default=28, type='int',
                      help='image width')
    parser.add_option('--height', dest='height', default=28, type='int',
                      help='image height')
...
def train_GAN(netD, netG, inital_epoch, args):
    ...
    if os.path.exists('./defensive_models_mnist/mnist_losses_gp.pickle'):

        with open ('./defensive_models_mnist/mnist_losses_gp.pickle', 'rb') as fp:
            save_losses = pickle.load(fp)

    if os.path.exists('./defensive_models_mnist/dev_disc_costs_mnist.pickle'):

        with open ('./defensive_models_mnist/dev_disc_costs_mnist.pickle', 'rb') as fp:
...
#             print(torch.randn(INPUT_LATENT))
            z = z.to(device_G)
...
#             print(fake_validity)
        ...
        if (iteration + 1) % display_steps == 0 or (iteration + 1) == ITERS:
            ...
            with open('./defensive_models_mnist/mnist_losses_gp.pickle', 'wb') as fp:
        ...
        if (iteration + 1) % 100 == 0 :
            ...
            g_path = './defensive_models_mnist/gen_mnist10_gp_' + str(iteration) + '.pth'
            d_path = './defensive_models_mnist/disc_mnist10_gp_' + str(iteration) + '.pth'
            
            modelG_copy = copy.deepcopy(netG)
            modelG_copy = modelG_copy.cpu()
            modelG_state_dict = modelG_copy.state_dict()
            torch.save(modelG_state_dict, g_path)

            modelD_copy = copy.deepcopy(netD)
            modelD_copy = modelD_copy.cpu()
            modelD_state_dict = modelD_copy.state_dict()
            torch.save(modelD_state_dict, d_path)
        ...
        if (iteration + 1) % 1000 == 0 :
            ...
            save_image(fake_imgs.data, './MNIST/samples_{}.png'.format(iteration), nrow=5, normalize=True)
            ...
            with open('./defensive_models_mnist/dev_disc_costs_mnist.pickle', 'wb') as fp:
...
if __name__ == "__main__":
    ...
    netD = Discriminator()
    summary(netD, input_size = (1, 28, 28), device = 'cpu')

    # set folder to save model checkpoints 
    model_folder = os.path.abspath('./defensive_models_mnist')
    if not os.path.exists(model_folder):
        os.mkdir(model_folder)

    check_point_path =  './defensive_models_mnist/snapshots.pth'
